...
public class UserRestFulController {
    ...
    /**
     *  用户注册  第一步  输入 用户名和密码 就行session 的存储，并且验证 用户名的 有效性
     *  经过和前端商量，验证邮箱的页面和注册用户的页面可以直接传递值，
     *  这里不再保存session信息，省的来回验证的麻烦
     *  这里只做 用户信息的 校验
     */
    /**注册*/
    ...
    public AjaxMsg regist(TSUser tsUser , HttpServletResponse response, HttpServletRequest request){//注册
        AjaxMsg ajaxMsg = new AjaxMsg();
        String msg = "success";
       String username = tsUser.getUserName();//注册的用户名字，即邮箱
       if(StringUtil.isEmpty(username)){
            ajaxMsg.setMsg("用户名不能为空!");
            return ajaxMsg;
        }
        ...
        //查询数据库是否 有该用户
        List<TSUser> list = systemService.findHql(" from TSUser where userName=?",new Object[]{username});
        if(list!=null && list.size()>0){//有该用户
            ajaxMsg.setMsg("该邮箱已经被注册!");
            return ajaxMsg;
        }
        //到这里，用户是正常的用户，没有注册
       // HttpSession session = request.getSession();
       // session.setAttribute("registuser",tsUser);//保存 临时注册的用户名和密码
       // JSONObject obj = new JSONObject();
        //obj.put("REGISTSESSIONID",session.getId());////前端的cookie 需要保存这个cookie，来和后台验证这个session的用户
       /// ajaxMsg.setModel(obj);//返回 REGISTSESSIONID




        ajaxMsg.setMsg(msg);//校验成功
        return ajaxMsg;
    }
     ...
     *注册
     * */
    public AjaxMsg doregist(TSUser tsUser , HttpServletResponse response, HttpServletRequest request){//注册
        AjaxMsg ajaxMsg = new AjaxMsg();
        String msg = "success";
        UserInfo userInfo = null;
        if(tsUser!=null){
    ...
    public AjaxMsg sendEmail(TSUser tsUser , HttpServletResponse response,HttpServletRequest request){
        ...
        //发送 邮件 这里 username 一定不为空
        Integer radomCode = (int)((Math.random()*9+1)*100000);
        UtilEmail.sendEmailForSht(userName,radomCode);
    ...
    public AjaxMsg checkEmail(TSUser tsUser , String code, HttpServletResponse response, HttpServletRequest request){
        AjaxMsg ajaxMsg = new AjaxMsg();
        //前端 cookie 传一个 EMAILSESSIONID 验证的时候 传过来
        ...
        if(StringUtil.isEmpty(codeSessionID)){//没有获取到
            ...
            if(session!=null){
                ...
                if(StringUtil.isEmpty(code)){//用户没有传过来验证码。
                    ...
                    String usertype = tsUser.getUserType();
                    if(StringUtil.isEmpty(usertype)){
                        ajaxMsg.setMsg("用户类别不能为空!");
                        return ajaxMsg;
                    }
                    //查询数据库是否 有该用户
                    List<TSUser> list = systemService.findHql(" from TSUser where userName=?",new Object[]{username});
                    ...
                    //到这里 ，用户名，密码 ，code 都验证成功了。开始注册
                    return  doregist(tsUser ,  response,  request);
    ...
    public AjaxMsg reSetPassWord(TSUser tsUser , String code, HttpServletResponse response, HttpServletRequest request){
        ...
        List list = systemService.findHql(" from TSUser where userName=? ", new Object[]{userName});
        if(list!=null && list.size()>0){
            TSUser tsUser1 = (TSUser)list.get(0);
            tsUser1.setPassword(newPassword);

            tsUser1.setPsd(password);// 修改明文密码
    ...
    public AjaxMsg reSetPassWordOnLine(TSUser tsUser , String code, HttpServletResponse response, HttpServletRequest request){
        ...
        List list = systemService.findHql(" from TSUser where userName=? and password=? ",new Object[]{userName, passwordP});
        TSUser tsUser1 = null;
        if(list!=null && list.size()>0){
        ...
        tsUser1.setPsd(newpassword);// 修改明文密码
