...
<script type="text/javascript">
	...
	function test (src){
	 	  //var rows = $("#t_txl").datagrid("getSelections");获取选中checkbox的值
	 	  //var rows = $("#dg").datagrid("getSelected");//返回第一个被选中的行 		选中行的时候
		  var rows = $("#t_txl").datagrid("getRows");
		  var row = rows[2];
		  var id = row.id;
	}
		$(function(){
			$("#t_txl").datagrid({
				...
				columns:[[    
					        ...
					        //{field:"txlgroupid",title:"所属分组",align:"center"},
					        ...
					        //{field:"upttime",title:"修改时间",align:"center"},
					        {field:"state",title:"状态",align:"center"}
					        /* {field:"opt",title:"操作",align:"center",
					        	formatter: function(value,row,index){
					        		return "<span onclick=\"deltxlConfirm(""+row.id+"","single");\" style=\"cursor: pointer;color:red;\">删除</span>&nbsp;"
					        		+"<span onclick=\"upttxl(""+row.id+"");\" style=\"cursor: pointer;color:red;\">修改</span>";
								}
					        } */
					     ]],
				loadMsg:"正在加载,请稍后……",
		...
		function addtxl(){
			$("#mydialog").dialog({    
			    ...
			    closed: false,    
			    cache: false,    
			    href: "<%=basePath%>txlController/goAddOrUptPage.do?load=add",    
			    modal: true,
			    buttons:[{
				text:"保存",
					handler:function(){
		...
		function seetxl(){
			//允许多行选中(但只能选中一条)
		    var ids = [];
		    var rows = $("#t_txl").datagrid("getSelections");
		    ...
		    if(idstr==null || idstr=="" || ids.length<1){
		    	$.messager.alert("警告","请选择一条数据！","warning");
		    	 ...
		    	 if(ids.length>1){
		    	 	...
		    	 	$("#mydialog").dialog({
					    href: "<%=basePath%>txlController/goAddOrUptPage.do?load=see&id="+idstr,
					    title: "查看",    
					    top:100,			    
			    		width: 400,    
			    		height: 260,    
		    	...
		    	$.messager.alert("提示","请选择一条数据！","warning");
		    	 ...
		    	 if(ids.length>1){
		    	 	...
		    	 	$("#mydialog").dialog({
					    ...
					    closed: false,    
					    cache: false,    
					    href: "<%=basePath%>txlController/goAddOrUptPage.do?load=upt&id="+ids,   
					    modal: true,
					     buttons:[{
						text:"保存",
							handler:function(){
								save();//调用子页面(txl_add.jsp)的save方法
							}
						},{
							text:"关闭",
							handler:function(){$("#mydialog").dialog("close"); }
		...
		function deltxl(){
			/*只允许单个选中
			var row = $("#t_txl").datagrid("getSelected");
			if (row){
				alert("Item ID:"+row.id+"txlname:"+row.txlname);
			}
			*/
			//允许多行选中
		    var ids = [];
		    ...
		    if(idstr==null || idstr=="" || ids.length<1){
		    	$.messager.alert("警告","请选择至少一条数据！","warning");
		    	 ...
		    	 deltxlConfirm(idstr,"more");
		    } 
		}
		//确认删除[msg==more多行删除][msg==single单行删除]
		function deltxlConfirm(idstr,msg){
			$.messager.confirm("确认","您确认想要删除记录吗？",function(r){
				if (r){  
			         $.ajax({
			        	url:"txlController/del.do",
			        	type:"POST",
			        	async:false,
			        	dataType:"json",
