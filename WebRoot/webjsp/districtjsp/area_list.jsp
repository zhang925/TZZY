...
<script type="text/javascript">
	...
	function test (src){
	 	  //var rows = $("#t_area").datagrid("getSelections");获取选中checkbox的值
	 	  //var rows = $("#dg").datagrid("getSelected");//返回第一个被选中的行 		选中行的时候
		  var rows = $("#t_area").datagrid("getRows");
		  var row = rows[2];
		  var id = row.id;
	}
		$(function(){
			$("#t_area").datagrid({
				...
				columns:[[    
					        ...
					        {field:"citycode",title:"市编码",align:"center"},
					        {field:"state",title:"状态",align:"center"}
					        /* {field:"opt",title:"操作",align:"center",
					        	formatter: function(value,row,index){
					        		return "<span onclick=\"delareaConfirm(""+row.id+"","single");\" style=\"cursor: pointer;color:red;\">删除</span>&nbsp;"
					        		+"<span onclick=\"uptarea(""+row.id+"");\" style=\"cursor: pointer;color:red;\">修改</span>";
								}
					        } */
					     ]],
				loadMsg:"正在加载,请稍后……",
				...
				toolbar: [{
						...
						text:"行政编码：<input id=\"pcode\" style=\"width:50px\" />"
							...
							+"市编码：<input id=\"pcitycode\"   style=\"width:50px\" />",
						handler: function(){}
					},"-",{
						iconCls: "icon-search",
			 ...
			 //设置分页控件  
		    var p = $("#t_area").datagrid("getPager"); 
		    $(p).pagination({  
		...
		function addarea(){
			$("#mydialog").dialog({    
			    ...
			    closed: false,    
			    cache: false,    
			    href: "<%=basePath%>areaController/goAddOrUptPage.do?load=add",    
			    modal: true,
			    buttons:[{
				text:"保存",
					handler:function(){
		...
		function seearea(){
			//允许多行选中(但只能选中一条)
		    var ids = [];
		    var rows = $("#t_area").datagrid("getSelections");
		    ...
		    if(idstr==null || idstr=="" || ids.length<1){
		    	$.messager.alert("警告","请选择一条数据！","warning");
		    	 ...
		    	 if(ids.length>1){
		    	 	...
		    	 	$("#mydialog").dialog({
					    href: "<%=basePath%>areaController/goAddOrUptPage.do?load=see&id="+idstr,
					    title: "查看",    
					    top:100,			    
			    		width: 400,    
			    		height: 260,    
		    	...
		    	$.messager.alert("提示","请选择一条数据！","warning");
		    	 ...
		    	 if(ids.length>1){
		    	 	...
		    	 	$("#mydialog").dialog({
					    ...
					    closed: false,    
					    cache: false,    
					    href: "<%=basePath%>areaController/goAddOrUptPage.do?load=upt&id="+ids,   
					    modal: true,
					     buttons:[{
						text:"保存",
							handler:function(){
								save();//调用子页面(area_add.jsp)的save方法
							}
						},{
							text:"关闭",
							handler:function(){$("#mydialog").dialog("close"); }
		...
		function delarea(){
			/*只允许单个选中
			var row = $("#t_area").datagrid("getSelected");
			if (row){
				alert("Item ID:"+row.id+"areaname:"+row.areaname);
			}
			*/
			//允许多行选中
		    var ids = [];
		    ...
		    if(idstr==null || idstr=="" || ids.length<1){
		    	$.messager.alert("警告","请选择至少一条数据！","warning");
		    	 ...
		    	 delareaConfirm(idstr,"more");
		    } 
		}
		//确认删除[msg==more多行删除][msg==single单行删除]
		function delareaConfirm(idstr,msg){
			$.messager.confirm("确认","您确认想要删除记录吗？",function(r){
				if (r){  
			         $.ajax({
			        	url:"areaController/del.do",
			        	type:"POST",
			        	async:false,
			        	dataType:"json",
